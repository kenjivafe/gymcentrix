...
interface ActivateBranchButtonProps {
  ...
  branchId: string;
  branchName: string;
  currentActiveBranchName?: string;
  className?: string;
}

export function ActivateBranchButton({ gymId, branchId, branchName, currentActiveBranchName, className }: ActivateBranchButtonProps) {
        ...
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300 cursor-default">
          <div className="w-full max-w-md bg-[#0A0A0A] border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="relative p-8">
              ...
              <h3 className="text-xl font-display font-bold text-white mb-2 tracking-tight">Activate Branch</h3>
