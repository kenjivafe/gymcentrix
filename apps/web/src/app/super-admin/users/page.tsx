...
import { UserPlus, Shield, Mail, Building2, ChevronRight, User } from "lucide-react";
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
           ...
           <p className="text-sm text-white/40">Manage administrative access across the entire platform.</p>
        </div>
      ...
      <div className="rounded-2xl border border-white/5 bg-white/[0.02] overflow-x-auto">
        {users.length === 0 ? (
          <div className="py-24 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-white/40 mt-2 text-sm max-w-sm">Create a new administrative user to get started.</p>
          </div>
        ) : (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-medium text-white/80">
                        {user.role === 'SUPER_ADMIN' 
                          ? 'Global Platform' 
                          : user.gymsOwned?.[0]?.name || "Unassigned"}
                      </span>
                    </div>
                  </td>
